#!/usr/bin/env node
/**
 * Sprint 2 Staged Tests (A → B → C) — Automation
 *
 * Stage A: All Sprint 2 flags off (baseline).
 * Stage B: Phase 2 only (scroll + carousel disabled).
 * Stage C: Phase 2 + Phase 3 (full Sprint 2 optimisations).
 *
 * Usage (from search/backend):
 *   npm run test:s2:staged       # all stages
 *   npm run test:s2:staged:a    # Stage A only
 *   npm run test:s2:staged:b    # Stage B only
 *   npm run test:s2:staged:c    # Stage C only
 *
...
// Sprint 2 Stage A: all Sprint 2 flags off (and Sprint 1 at baseline)
const STAGE_A_ENV = {
...
// Stage B: Phase 2 only (scroll + carousel off)
const STAGE_B_ENV = {
  ...STAGE_A_ENV,
...
// Stage C: Full Sprint 2 (Phase 2 + Phase 3)
const STAGE_C_ENV = {
  ...STAGE_B_ENV,
...
async function runStage(stage, opts) {
  ...
  const logPath = opts.logFile || path.join(__dirname, `sprint2-stage-${stage}-${Date.now()}.log`);
  let backendProc = null;
  let logFd = null;
  let logPathToRead = logPath;

  if (!opts.backendRunning) {
    const label = stage === 'a' ? 'Sprint 2 baseline' : stage === 'b' ? 'Phase 2 only' : 'Phase 2+3';
  ...
  const correlationIds = urls.map((_, i) => `s2-${stage}-${Date.now()}-${i}`);
  const responses = [];
  ...
  const envSummary =
    stage === 'a'
      ? 'All Sprint 2 flags off'
      : stage === 'b'
        ? 'Phase 2 only (scroll+carousel off)'
        : 'Full Sprint 2 (Phase 2+3)';
...
const REPORT_PATH = path.resolve(
  ...
  '06-sprint2-staged-test-results.md'
);
