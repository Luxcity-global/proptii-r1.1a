...
import { scrape, scrapeInternet, scrapeInternetFast } from './scraper';
...
// Ultra-fast internet search endpoint - returns basic results in under 10 seconds
app.post('/scrape-internet-fast', async (req, res) => {
  try {
    const { query } = req.body;
    ...
    console.log('Ultra-fast internet search query:', query);
    
    // Set response timeout to 15 seconds max
    ...
    try {
      // Use the fast scrapeInternetFast function from scraper.ts
      const results = await scrapeInternetFast(query, apiKey);
    ...
    console.error('Error in /scrape-internet-fast endpoint:', error);
    if (!res.headersSent) {
      res.status(500).json({
        error: 'Internal server error',
        message: error instanceof Error ? error.message : 'An unexpected error occurred'
  ...
  try {
    ...
    if (url.includes('openrent.co.uk')) {
      ...
      try {
        ...
        // OnTheMarket ONLY - no fallback to other sites
        console.log('OnTheMarket scraper failed - returning empty results (OnTheMarket only)');
        return res.json([]);
      }
    }
  } catch (error) {
