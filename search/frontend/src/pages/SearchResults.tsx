...
function SearchResults() {
  ...
  const handleSearch = async (searchQuery: string = newQuery) => {
    ...
    setResults([]);
    setCurrentPage(1);
    setLoadingMessage(searchType === 'internet' ? 'Finding property listings...' : 'Searching...');
    ...
    try {
      ...
      if (searchType === 'internet') {
        endpoint = `${backendUrl}/scrape-internet-fast`;
        requestBody = { 
          query: searchQuery
        };
      } else {
      ...
      // Show different completion message based on search type
      if (searchType === 'internet' && cleanedData.length > 0) {
        setLoadingMessage('Properties found! Loading contact details...');
        setTimeout(() => setLoading(false), 500); // Brief delay to show message
      }
    } catch (error) {
      setError(error instanceof Error ? error.message : 'An unexpected error occurred');
      setResults([]);
    } finally {
      if (!(searchType === 'internet' && results.length > 0)) {
        setLoading(false);
      }
    }
  };
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p style={{ color: '#E65D24' }} className="font-medium">{loadingMessage}</p>
            {searchType === 'internet' && (
              <p className="text-gray-500 text-sm mt-2">Fast results loading...</p>
            )}
          </div>
        )}
