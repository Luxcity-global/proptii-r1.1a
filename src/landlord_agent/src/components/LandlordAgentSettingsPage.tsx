...
import type { UserProfile, UserRole } from '../App';
...
function pricingSegmentForRole(role: UserRole): 'landlords' | 'agents' {
  return role === 'agent' ? 'agents' : 'landlords';
}
...
const SettingsGroup: React.FC<SettingsGroupProps> = ({ label, icon, children }) => (
  <div
    className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-0 md:gap-8 py-8 border-b last:border-b-0"
...
interface LandlordAgentSettingsPageProps {
...
const LandlordAgentSettingsPage: React.FC<LandlordAgentSettingsPageProps> = ({
  ...
  const planConfig = getPlanById(displayPlanId);
  ...
  const upgradeDescription =
    userRole === 'agent'
      ? 'Unlock fit checks, client management, referencing, and more with an Agent plan.'
      : 'Unlock property management, tenant referencing, contracts, and more with a Landlord plan.';
  ...
  return (
    <div style={{ fontFamily: 'Archivo, sans-serif' }}>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            {isFreePlan ? (
              ...
              <>
                <SettingsRow title="Billing cycle" description={cycleLabel} />
                {status === 'trialing' && trialEndsAt && (
                  <SettingsRow title="Trial ends" description={formatDate(trialEndsAt)} />
                )}
                ...
                {fitChecksQuota != null && (
                  <SettingsRow
                    title="Fit checks this period"
                    description={`${fitChecksUsed ?? 0} of ${fitChecksQuota} used`}
                  />
                )}
                {hasStripeCustomer && (
                  <SettingsRow
              ...
              <SettingsRow
                ...
                action={
                  portalBusy ? (
                    ...
                    <ChevronRight
                      className="flex-shrink-0 w-4 h-4 group-hover:translate-x-0.5 transition-transform"
                      style={{ color: '#9ca3af' }}
                    />
                  )
                }
      ...
      {showPlansModal && (
        <LandlordPlanCompareModal
          open={showPlansModal}
          onClose={() => setShowPlansModal(false)}
          currentPlanId={displayPlanId}
          defaultAudience={pricingSegment}
        />
      )}
    </div>
  );
...
export default LandlordAgentSettingsPage;
