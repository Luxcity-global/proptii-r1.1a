...
export const mockGetSavedProperties = async (): Promise<ApiResponse<SavedProperty[]>> => {
  try {
    ...
    // Get external collections saved properties from localStorage
    let externalSavedProperties: SavedProperty[] = [];
    try {
      const saved = localStorage.getItem('externalCollections_savedProperties');
      if (saved) {
        externalSavedProperties = JSON.parse(saved);
      }
    } catch (error) {
      console.error('Error loading external saved properties:', error);
    }
    
    // Combine original mock data with external collections saved properties
    const allSavedProperties = [...mockSavedProperties, ...externalSavedProperties];
    
    return { success: true, data: allSavedProperties };
  } catch (error: any) {
    console.error('Mock API Error:', error);
    return {
      success: false,
