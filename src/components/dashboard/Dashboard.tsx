...
export const DASHBOARD_SECTIONS = [
  {
    id: 'dashboard', label: 'Dashboard', path: '/dashboard', icon: (isSelected: boolean) => (
      <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
  ...
  {
    id: 'saved-searches', label: 'Saved Searches', path: '/dashboard/saved-searches', icon: (isSelected: boolean) => (
      <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
      ...
      >
        <g clipPath="url(#clip0_1919_20172)">
          <path
            d="M6.5 4.482c1.664-1.673 5.825 1.254 0 5.018-5.825-3.764-1.664-6.69 0-5.018" fill={isSelected ? "#003450" : "#374957"} />
  ...
  {
    id: 'tenant-referencing', label: 'Tenant Referencing', path: '/dashboard/tenant-referencing', icon: (isSelected: boolean) => (
      <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
  ...
  {
    id: 'your-files', label: 'Your Files', path: '/dashboard/your-files', icon: (isSelected: boolean) => (
      <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
  ...
  {
    id: 'messages', label: 'Messages', path: '/dashboard/messages', icon: (isSelected: boolean) => (
      <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
  ...
  { id: 'settings', label: 'Settings', path: '/dashboard/settings', icon: (isSelected: boolean) => (
    <svg
      width="20"
      height="20"
      viewBox="0 0 24 24"
    ...
    >
      <path
        ...
        stroke={isSelected ? '#003450' : '#374957'}
        strokeWidth="1.5"
        strokeLinecap="round"
    ...
    >
      ...
      {isMobile && (
        <>
          ...
          {/* Sidebar */}
          <div
            className={`fixed inset-y-0 left-0 z-50 w-[280px] bg-white border-r transition-transform duration-300 ease-out md:hidden ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'
              }`}
      ...
      {/* Main content — sidebar spacer already offsets layout; no extra marginLeft */}
      <main
...
/**
 * Dashboard component for tenant portal.
 * Wraps DashboardInner with MessagingProvider so that all child routes have
 * access to the messaging context and the 30-second polling loop.
 */
